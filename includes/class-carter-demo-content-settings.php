 ...
 */
class Carter_Demo_Content_Settings {
	...
	public static function get_settings() {
		...
		return wp_parse_args( get_option( 'carter_theme_settings', array() ), $default_settings );
	}
	...
	public static function register_settings() {
		add_settings_field(
			'carter_theme_settings[carter_demo_content]',
			esc_html__( 'Import Content', 'carter' ),
			...
			array(
				'options' => array(
					...
					'posts'      => sprintf( esc_html__( 'Blog Posts (%s)', 'carter' ), 5 ),
					/* translators: number of categories to be imported */
					'categories' => sprintf( esc_html__( 'Categories (%s)', 'carter' ), 3 ),
					/* translators: number of imges to be imported */
					'image'      => sprintf( esc_html__( 'Featured Images (%s)', 'carter' ), 1 ),
					/* translators: number of static pages to be imported */
					'pages'      => sprintf( esc_html__( 'Static Pages (%s)', 'carter' ), 4 ),
				),
			)
		);
	}
	...
	public static function render_demo_content_setting( $args ) {
		...
		$demo_content = $options['carter_demo_content'];

		$html = '';
		if ( ! empty( $args['options'] ) ) :
			foreach ( $args['options'] as $key => $option ) {
				$checked = isset( $demo_content[ $key ] ) ? checked( 1, $demo_content[ $key ], false ) : '';
				$setting = 'carter_theme_settings[carter_demo_content][' . $key . ']';
	...
	public static function import_demo_content() {
		...
		// Return early if not on Carter admin page.
		if ( ! isset( $_POST['carter_theme_settings'] ) ) {
			return;
		}

		// Listen for our import button to be clicked.
		if ( ! isset( $_POST['carter_import_demo_content'] ) ) {
			return;
		}

		// Run a quick security check.
		if ( ! check_admin_referer( 'carter_demo_content_nonce', 'carter_demo_content_nonce' ) ) {
			return;
		}

		$categories = array();
		if ( ! empty( $_POST['carter_theme_settings']['carter_demo_content']['categories'] ) ) {
			try {
		...
		$image_id = 0;
		if ( ! empty( $_POST['carter_theme_settings']['carter_demo_content']['image'] ) ) {
			try {
		...
		if ( ! empty( $_POST['carter_theme_settings']['carter_demo_content']['posts'] ) ) {
			try {
		...
		if ( ! empty( $_POST['carter_theme_settings']['carter_demo_content']['pages'] ) ) {
			try {
		...
		wp_safe_redirect( admin_url( 'themes.php?page=carter-theme' ) );
	}
	...
	private static function create_demo_image() {
		$file = 'https://preview.themezee.com/wp-content/themes/carter/assets/img/default-image.png';

		$file_array = array(
			'name'     => 'carter-demo-image.png',
...
// Run class.
Carter_Demo_Content_Settings::setup();
